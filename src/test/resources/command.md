...
```
curl.exe -i -X POST http://127.0.0.1:8080/inbound/save/voice -F "externalId=voice_002" -F "file=@src/test/resources/voice.m4a" -w "`nHTTP %{http_code} - took %{time_total}s`n"
```
...
```
curl.exe -s http://127.0.0.1:8080/expenses | ConvertFrom-Json | Format-Table amount, currency, category, merchant, confidence
```
